...
本文件由 agent-Joshu 主包模板提供，安装时并入项目根 AGENTS.md（放在 Trellis 管理块之外）。
...
- 任务过程：Trellis 任务在 `.trellis/`（本地，已被 gitignore）；无 Trellis 时建普通任务目录（`TASK.md`、`log.md`、`sources/`、`results/`），同样不进 git。
