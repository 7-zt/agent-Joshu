...
### 2026-09-21 移除 tools/check-workflow.ps1（Linux 适配）

用户决定删除 Windows PowerShell 单平台的自检脚本及其 tools/ 目录，考虑 Linux 设备适配。后果：结构/链接/ADR/模板/绝对路径自检暂无自动化，回归时按 tests/ 场景卡人工核对；后续如需跨平台自检（如 Python/bash 实现）另提提案。
